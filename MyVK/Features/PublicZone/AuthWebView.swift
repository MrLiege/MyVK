...
//  MyVK
//
//  Created by Artyom Petrichenko on 04.09.2024.
//
...
extension AuthWebView {
    final class Coordinator: NSObject, WKNavigationDelegate {
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            guard let url = webView.url, url.absoluteString.contains("access_token") else { return }
            
            guard let creds = getCredentials(from: url) else { return }
            
            UserStorage.shared.token = creds.token
            UserStorage.shared.userID = creds.userID
        }
    }
}

extension AuthWebView.Coordinator {
    private func getCredentials(from url: URL) -> AuthCredentials? {
        ...
        guard let userIdRange = strings.last?.range(of: Consts.userID.appending("=")) else {
            return creds
        }
